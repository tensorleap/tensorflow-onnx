...
import tf2onnxnightly.onnx_opset  # pylint: disable=unused-import
import tf2onnxnightly.tflite_handlers  # pylint: disable=unused-import
import tf2onnxnightly.custom_opsets  # pylint: disable=unused-import
from tf2onnxnightly.graph import Graph
from tf2onnxnightly.rewriter import *  # pylint: disable=wildcard-import
from tf2onnxnightly.tflite_rewriters import *  # pylint: disable=wildcard-import
from tf2onnxnightly.shape_inference import infer_shape
from tf2onnxnightly.tf_loader import is_function, resolve_functions, set_function
from tf2onnxnightly.tf_utils import tensorflow_to_onnx, get_tf_version, compute_const_folding_using_tf
            ...
            try:
                ...
                if inputs and len(op.input) == len(inputs):
                    ...
                    if op.type == "Cast":
                        ...
                        np_type = tf2onnxnightly.utils.map_onnx_to_numpy_type(out_type)
                        val = func(*inputs)
                        ...
                        np_type = tf2onnxnightly.utils.map_onnx_to_numpy_type(dtype)
...
def process_tf_graph(tf_graph, continue_on_error=False, verbose=False, target=None,
    ...
    if not is_subgraph:
        logger.info("Using tensorflow=%s, onnx=%s, tf2onnx=%s/%s",
                    get_tf_version(), utils.get_onnx_version(), tf2onnxnightly.__version__, tf2onnxnightly.version.git_version[:6])
...
def tf_optimize(input_names, output_names, graph_def, fold_constant=True):
    ...
    return tf2onnxnightly.tf_loader.tf_optimize(input_names, output_names, graph_def, fold_constant)
